...
export const getAllCatatan = async (request: Request, response: Response) => {
  try {
    const allCatatan = await prisma.catatan.findMany({});
    const totalIncome = await prisma.catatan.aggregate({
      _sum: { amount: true },
      where: { type: "INCOME" },
    });
    const totalExpense = await prisma.catatan.aggregate({
      _sum: { amount: true },
      where: { type: "EXPENSE" },
    });
...
export const renderAddCatatan = async (request: Request, response: Response) => {
  return response.render("add");
};

export const renderEditCatatan = async (request: Request, response: Response) => {
  try {
    const { id } = request.params;
    const findCatatan = await prisma.catatan.findFirst({
      where: { id: Number(id) },
    ...
    if (!findCatatan) {
      return response.status(404).render("edit", {
        catatan: null,
        message: "Catatan tidak ditemukan",
      });
    }
    return response.render("edit", { catatan: findCatatan });
  } catch (error) {
    return response.status(400).render("edit", {
      catatan: null,
      message: `Terjadi kesalahan: ${error}`,
    });
  }
...
export const createCatatan = async (request: Request, response: Response) => {
  try {
    ...
    const newCatatan = await prisma.catatan.create({
      data: { id: Number(id), type, amount: Number(amount), description },
    });

    return response.redirect("/catatan");
  } catch (error) {
...
export const hitungTotalCatatan = async (request: Request, response: Response) => {
  try {
    ...
    if (totalAmount._sum.amount === null) {
      return response.status(404).json({
        message: "Tidak ada transaksi yang ditemukan untuk tipe ini.",
      });
    }
    ...
    return response.status(500).json({
      message: `Terjadi kesalahan saat menghitung total: ${error}`,
    });
  }
};
